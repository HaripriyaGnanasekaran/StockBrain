...
pf_return = []
pf_risk = []
stocklist=['GOOG','SPY','RDS-A','BABA','AMZN','ENPH','TSLA','MSFT','MA','AAPL']
...
for runs in range(1,2):
    ...
    dictionary = {'Stock':stocklist,'weight':opt_weight,'return':avg_return,'std':avg_std}
    ...
    for i,name in enumerate(stocklist):
        df_port_cov[name]=df_cov[name]*df_cov['weight']*df_cov.loc[i,'weight']
